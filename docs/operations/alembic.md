...
- деструктивные-операции разделяются на совместимые этапы, если старая и новая
  версии приложения могут работать одновременно.
- миграцию нужно проверить на PostGIS БД до merge.

Продовый флоу применяет `upgrade head` перед перезапуском приложения.
Автоматического даунгрейда при роллбеке образа нет, совместимость и план отката
